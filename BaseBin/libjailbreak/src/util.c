...
static bool sptm_capability_incomplete(void)
{
	return jbinfo_has_sptm_metadata() && !jbinfo_sptm_runtime_ready();
}

uint32_t sptm_frame_get_refcnt_off(uint64_t frame)
{
	if (sptm_capability_incomplete() || frame == 0) return 0;
...
{
	if (sptm_capability_incomplete() || pt_pa == 0) return 0;
...
{
	if (sptm_capability_incomplete() || pt_pa == 0) return;
	if (ksymbol(libsptm_frame_table)) {
		uint64_t sptmFrame = pa_to_sptm_frame(pt_pa);
		if (!sptmFrame) return;
		uint64_t refcntOff = sptm_frame_get_refcnt_off(sptmFrame);
...
{
	if (delta == 0 || sptm_capability_incomplete() || pt_pa == 0) return;
...
{
	if (sptm_capability_incomplete() || pt_pa == 0 || pmap == 0) return;
	uint64_t pvh = pai_to_pvh(pa_index(pt_pa));
	uint64_t ptdp = pvh_ptd(pvh);
	if (!ptdp) return;
...
{
	if (sptm_capability_incomplete() || pt_pa == 0 || vm_real_kernel_page_size == 0) return;
	uint64_t pvh = pai_to_pvh(pa_index(pt_pa));
	uint64_t ptdp = pvh_ptd(pvh);
	if (!ptdp) return;
	uint64_t ptdp_pa = kvtophys(ptdp);
...
{
	if (sptm_capability_incomplete()) return 0;
...
{
	if (sptm_capability_incomplete()) return -ENOTSUP;
	if (!pmap || size == 0 || size > UINT64_MAX - vaStart) return -EINVAL;
	uint64_t ttep = kread64(pmap + koffsetof(pmap, ttep));
	if (!ttep) return -EIO;
...
{
	if (sptm_capability_incomplete()) return -ENOTSUP;
	if (!pmap || size == 0 || size > UINT64_MAX - uaStart || size > UINT64_MAX - paStart ||
	    vm_real_kernel_page_size == 0 || (uaStart & vm_real_kernel_page_mask) ||
	    (paStart & vm_real_kernel_page_mask) || (size & vm_real_kernel_page_mask)) return -EINVAL;
	uint64_t ttep = kread64(pmap + koffsetof(pmap, ttep));
	if (!ttep) return -EIO;
...
int __exec_cmd_internal_va(bool suspended, bool root, bool waitForExit, pid_t *pidOut, const char *binary, int argc, va_list va_args, char **envp)
{
	...
	pid_t spawnedPid = 0;
	/* RootHide owns process creation here: it applies the fork's hiding and
	 * injection policy before the child can observe its environment. */
