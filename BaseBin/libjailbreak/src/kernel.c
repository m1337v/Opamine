...
uint64_t pai_to_pvh(uint64_t pai)
{
	if (__builtin_available(iOS 27.0, *)) {
		if (jbinfo_has_sptm_metadata()) {
			if (!jbinfo_sptm_runtime_ready()) return 0;
...
{
	if (!jbinfo_sptm_runtime_ready() || !ksymbol_sptm(n_papt_ranges_compressed) ||
	    !ksymbol_sptm(papt_ranges_compressed)) return 0;
	uint32_t count = kread32(ksymbol_sptm(n_papt_ranges_compressed));
	if (count == 0 || count > 4096) return 0;
	struct papt_range_compressed *ranges = calloc(count, sizeof(*ranges));
	if (!ranges) return 0;
	if (kreadbuf(ksymbol_sptm(papt_ranges_compressed), ranges, count * sizeof(*ranges)) != 0) {
		free(ranges);
		return 0;
	}
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		uint64_t length = (uint64_t)range->numPages << 14;
		if (length == 0 || range->startAddr > UINT64_MAX - length || user_root_pt < range->startAddr ||
		    user_root_pt >= range->startAddr + length) continue;
		uint64_t surt = user_root_pt - range->startAddr + range->baseAddr;
		if (__builtin_available(iOS 27.0, *)) {
			result = (surt & 0xFFFFFFFFFFFFC000ULL) |
...
{
	if (!jbinfo_sptm_runtime_ready() || !ksymbol(libsptm_frame_table)) return 0;
	uint64_t frameTable = kread64(ksymbol(libsptm_frame_table));
	if (!frameTable) return 0;
	uint64_t candidate = frameTable + (pa_index(pa) * 16);
...
{
	if (pvh == 0 || kconstant(PVH_HIGH_FLAGS) == 0) return 0;
...
int pmap_cs_allow_invalid(uint64_t pmap)
{
	...
	if (koffsetof(pmap, txm_address_space)) {
		if (!jbinfo_sptm_runtime_ready() || !koffsetof(TXMAddressSpace, allowsInvalidCode)) return -1;
		uint64_t txm_address_space = kread_ptr(pmap + koffsetof(pmap, txm_address_space));
		if (!txm_address_space) return -1;
		return kwrite8(txm_address_space + koffsetof(TXMAddressSpace, allowsInvalidCode), true);
	}
...
{
	if (!jbinfo_sptm_runtime_ready() || !ksymbol(pmap_first_pnum) || !ksymbol(vm_pages_radix_root) ||
	    kconstant(VM_PAGE_PACKED_PTR_SHIFT) == 0 || kconstant(VM_PAGE_PACKED_PTR_BASE) == 0) return 0;
