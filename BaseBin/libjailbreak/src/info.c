...
void jbinfo_initialize_hardcoded_offsets(void)
{
	...
	const char *xnuString = strstr(name.version, "xnu-");
	if (!xnuString) {
		fprintf(stderr, "libjailbreak: unsupported kernel version format; leaving dynamic capabilities disabled\n");
		return;
	}
	int parsedXnuComponents = sscanf(xnuString, "xnu-%llu.%llu.%llu~%*s", &xnuVersion.major, &xnuVersion.minor, &xnuVersion.patch);
	if (parsedXnuComponents != 3) {
		parsedXnuComponents = sscanf(xnuString, "xnu-%llu.%llu.%*s", &xnuVersion.major, &xnuVersion.minor);
		if (parsedXnuComponents != 2) {
			fprintf(stderr, "libjailbreak: unsupported XNU version; leaving dynamic capabilities disabled\n");
			return;
		}
	}
	uint64_t darwinMajor = 0;
	if (sscanf(darwinVersion, "%llu", &darwinMajor) != 1 || darwinMajor < 21 || darwinMajor > 27) {
		fprintf(stderr, "libjailbreak: unsupported Darwin release; leaving dynamic capabilities disabled\n");
		return;
	}
...
void jbinfo_initialize_boot_constants(void)
{
	...
	if (jbinfo_has_sptm_metadata()) {
		/* A legacy one-argument XPF startup only supplies the kernel image.
		 * Do not derive SPTM/TXM slides from zero static bases: every caller must
		 * then stay on the non-SPTM path instead of issuing malformed accesses. */
		if (kconstant(staticSptmBase) == 0 || kconstant(staticTxmBase) == 0) {
			fprintf(stderr, "libjailbreak: SPTM metadata present without image bases; disabling SPTM/TXM paths\n");
			return;
		}

		uint64_t args = kread64(ksymbol(SPTMArgs));
		if (args == 0) {
			fprintf(stderr, "libjailbreak: unable to read SPTM arguments; disabling SPTM/TXM paths\n");
			return;
		}
		uint64_t debugHeaderAddr = kread64(args + 0x60);
		if (debugHeaderAddr == 0) {
			fprintf(stderr, "libjailbreak: SPTM debug header unavailable; disabling SPTM/TXM paths\n");
			return;
		}
		uint64_t sptmBase = kread64(debugHeaderAddr + 0x10);
		uint64_t txmBase = kread64(debugHeaderAddr + 0x20);
		if (sptmBase == 0 || txmBase == 0) {
			fprintf(stderr, "libjailbreak: SPTM/TXM bases unavailable; disabling SPTM/TXM paths\n");
			return;
		}
		gSystemInfo.kernelConstant.sptmBase = sptmBase;
		gSystemInfo.kernelConstant.txmBase = txmBase;
		gSystemInfo.kernelConstant.sptmSlide = sptmBase - kconstant(staticSptmBase);
		gSystemInfo.kernelConstant.txmSlide = txmBase - kconstant(staticTxmBase);
	}
}
