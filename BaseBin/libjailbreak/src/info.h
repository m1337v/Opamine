...
/* An SPTM metric alone is not enough.  A caller that still uses the legacy
 * one-argument XPF entry point has no static SPTM/TXM image bases and must not
 * enter the SPTM/TXM code paths. */
static inline bool jbinfo_has_sptm_metadata(void)
{
	return ksymbol(SPTMArgs) != 0;
}

static inline bool jbinfo_sptm_runtime_ready(void)
{
	return jbinfo_has_sptm_metadata() &&
	       kconstant(staticSptmBase) != 0 &&
	       kconstant(staticTxmBase) != 0 &&
	       kconstant(sptmBase) != 0 &&
	       kconstant(txmBase) != 0;
}
